...
class OnboardingAgent:
    ...
    def __init__(self):
        ...
        self.agent = Agent(
            ...
            instructions=(
                ...
                "1. When the user provides a Group Code, use the validate_group_code tool. If they say 'skip' or similar, use the create_new_group tool.\n"
                "2. Ask about the user's Gmail or Outlook address, then use the scan_email_for_profile tool "
            ...
            model="o4-mini",
            tools=[
                self._scan_email_for_profile_tool,
                self._send_group_invites_tool,
                self._validate_group_code_tool,
                self._create_new_group_tool
            ]
        )
    ...
    @function_tool
    def _scan_email_for_profile_tool(self, email: str) -> dict:
        """
        ...
        try:
            ...
            self.current_collected_data["email"] = email
    ...
    @function_tool
    def _send_group_invites_tool(self, email_addresses: str) -> dict:
        """
        Send group invitation emails
        """
        try:
            group_code = self.current_collected_data.get("group_code")
            sender_name = self.current_collected_data.get("name", "Arny User")
            sender_email = self.current_collected_data.get("email", "")
            
            if not group_code:
                return {"success": False, "error": "No group code available"}
            
            # Validate email addresses
            valid_emails = self.email_service.validate_email_addresses(email_addresses)
    ...
    @function_tool
    def _validate_group_code_tool(self, group_code: str) -> dict:
        """
        ...
        try:
            ...
            # Validate format
            formatted_code = self.group_generator.format_group_code(group_code)
            is_valid = self.group_generator.validate_group_code(formatted_code)
            ...
            group_exists = True  # This would be: await self.db.check_group_exists(formatted_code)
            
            if group_exists:
                # Store group info
                self.current_collected_data["group_code"] = formatted_code
                self.current_collected_data["group_role"] = "member"
    ...
    @function_tool
    def _create_new_group_tool(self) -> dict:
        """
        Create a new group for the user
        """
        try:
            # Generate unique group code
            new_group_code = self.group_generator.generate_group_code()
            
            # Store group info
            self.current_collected_data["group_code"] = new_group_code
            self.current_collected_data["group_role"] = "admin"
